...
export const DigitalSerenityBackground = () => {
    ...
    const handleScroll = () => {
      if (!scrolled) {
        ...
        floatingElementsRef.current.forEach((el, index) => {
          setTimeout(() => {
            if (el) {
              el.setAttribute('style', el.getAttribute('style') + 'animation-play-state: running; opacity: 1;');
            }
          }, (parseFloat(el.getAttribute('style')?.match(/animation-delay:\s*([\d.]+)s/)?.[1] || "0") * 1000) + index * 100);
        });
      }
    };
